function postComment(button, postedBy, videoId, replyTo, containerClass) {
    ...
    if(commentText) {
        ...
        .done(function(comment){
        	...
        	if(!replyTo) {
        		$("." + containerClass).prepend(comment);	//adding the comment as an answer
        	}
            else {
            	$(button).parent().siblings("." + containerClass).append(comment); //adding the comment as a new one
            }
        });
...
function toggleReply(button) {
    var parent = $(button).closest(".itemContainer"); /*jquery function that gets the first instance of the "itemContainer" class 
                                                        (searches parent element) */
    var commentForm = parent.find(".commentForm").first(); //"find" goes into the children elements
...
function getReplies(commentId, button, videoId) {
	$.post("ajax/getCommentReplies.php", { commentId: commentId, videoId: videoId}) //gets comments
