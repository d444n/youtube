...
class Video {
    ...
    public function __construct($con, $input, $userLoggedInObj) { //$input can be an id or sqlData
        $this->con = $con;
        $this->userLoggedInObj = $userLoggedInObj;

        if(is_array($input)) { // if its an array we already have the sqlData
            $this->sqlData = $input;
        }
        else {
            $query = $this->con->prepare("SELECT * FROM videos WHERE id = :id"); //if its an id we have to get the sqlData
    ...
    public function getId() { //functions we need to get video attributes
    ...
    public function incrementViews() { //increasing the view count
        ...
        $this->sqlData["views"] = $this->sqlData["views"] + 1; //sqlData views (the old one) has still the old views, altough db is updated
    }
    ...
    public function like() {
        ...
        if($this->wasLikedBy()) {
            // User has already liked, removing the like when clicked on like again
